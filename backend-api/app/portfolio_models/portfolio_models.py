...
def _prepare_latest_price_series(tickers, latest_prices_dict, reference_prices=None):
    ...
    if reference_prices is not None:
        try:
            if (
                isinstance(reference_prices, pd.DataFrame)
                and not reference_prices.empty
            ):
            ...
            Exception
        ) as exc:  # pragma: no cover - chỉ log khi có sự cố dữ liệu bất thường
            logger.warning(f"Khong the tao fallback price series: {exc}")
            fallback_series = pd.Series(dtype=float)

    overlap = [
        t for t in tickers if t in price_series.index and t in fallback_series.index
    ]
    ratios = []
    ...
    for ticker in tickers:
        ...
        if not _needs_fill(current_val):
            continue
        fallback_val = (
            fallback_series.get(ticker) if not fallback_series.empty else None
        )
...
def optimize_hrp_allocation(target_weights, prices, total_investment):
    ...
    logger.info(
        f"Sau khi lam tron: Tong chi: {total_spent:,.0f} VND, Con lai: {remaining:,.0f} VND"
    )
    ...
    while improved and iteration < max_iterations:
        ...
        for idx in priority_indices:
            if remaining >= price_arr[idx]:
                ...
                improved = True
                logger.info(
                    f"Mua them 1 {tickers[idx]} voi gia {price_arr[idx]:,.0f} VND"
                )
                break

    # Tạo dictionary kết quả
    allocation = {
        tickers[i]: int(shares_optimal[i]) for i in range(n) if shares_optimal[i] > 0
    }
    leftover = remaining

    # Validation
    total_spent_final = sum(
        allocation[ticker] * prices[ticker] for ticker in allocation
    )
    ...
    for ticker in tickers:
        ...
        diff = actual - target
        logger.info(
            f"{ticker}: Muc tieu={target:.4f}, Thuc te={actual:.4f}, Chenh lech={diff:+.4f}"
        )

    # Tính tổng bình phương sai số
    total_squared_error = sum(
        (actual_weights.get(t, 0) - target_weights.get(t, 0)) ** 2 for t in tickers
    )
...
def run_integer_programming(weights, latest_prices, total_portfolio_value):
    ...
    if total_portfolio_value <= 0:
        logger.error(
            f"So tien dau tu phai lon hon 0. Nhan duoc: {total_portfolio_value:,.0f} VND"
        )
        raise ValueError(
            f"So tien dau tu phai lon hon 0. Nhan duoc: {total_portfolio_value:,.0f} VND"
        )
    ...
    logger.info(
        f"Tong trong so: {sum(weights.values()) if isinstance(weights, dict) else weights.sum():.6f}"
    )
    logger.info(
        f"Gia co phieu nhan duoc: {latest_prices.to_dict() if isinstance(latest_prices, pd.Series) else latest_prices}"
    )
    ...
    total_spent = sum(
        allocation_lp[ticker] * latest_prices[ticker] for ticker in allocation_lp
    )
    ...
    if abs(total_check - total_portfolio_value) > 1:  # Cho phép sai số 1 VND
        logger.warning(
            f"Co su chenh lech: {abs(total_check - total_portfolio_value):,.0f} VND"
        )
    else:
...
def max_sharpe(data, total_investment, get_latest_prices_func, risk_free_rate=0.02):
    ...
    try:
        ...
        if abs(total_weight - 1.0) > 1e-5:
            logger.warning(
                f"[MAX_SHARPE] Tong trong so truoc khi chuan hoa: {total_weight}"
            )
            cleaned_weights = {
                k: v / total_weight for k, v in cleaned_weights.items() if v > 1e-5
            }
            logger.info(
                f"[MAX_SHARPE] Da chuan hoa lai trong so. Tong moi: {sum(cleaned_weights.values())}"
            )
        ...
        latest_prices_series = _prepare_latest_price_series(
            tickers, latest_prices_raw, data
        )
        latest_prices = latest_prices_series.to_dict()
        latest_prices_series = _prepare_latest_price_series(
            tickers, latest_prices, data
        )
        ...
        allocation_lp, leftover_lp = run_integer_programming(
            cleaned_weights, latest_prices_series, total_portfolio_value
        )
...
def min_volatility(data, total_investment, get_latest_prices_func):
    ...
    try:
        ...
        if abs(total_weight - 1.0) > 1e-5:
            logger.warning(
                f"[MIN_VOLATILITY] Tong trong so truoc khi chuan hoa: {total_weight}"
            )
            cleaned_weights = {
                k: v / total_weight for k, v in cleaned_weights.items() if v > 1e-5
            }
            logger.info(
                f"[MIN_VOLATILITY] Da chuan hoa lai trong so. Tong moi: {sum(cleaned_weights.values())}"
            )
...
def min_cvar(data, total_investment, get_latest_prices_func, beta=0.95):
    ...
    try:
        ...
        active_tickers = [
            ticker for ticker, weight in full_weights.items() if weight > 0
        ]
        ...
        portfolio_var = float(
            np.dot(weights_array.T, np.dot(cov_subset, weights_array))
        )
        ...
        allocation_lp, leftover_lp = run_integer_programming(
            full_weights, latest_prices_series, total_portfolio_value
        )
...
def hrp_model(data, total_investment, get_latest_prices_func):
    ...
    try:
        ...
        if abs(total_weight - 1.0) > 1e-5:
            logger.warning(
                f"[HRP_MODEL] Tong trong so truoc khi chuan hoa: {total_weight}"
            )
            cleaned_weights = {k: v / total_weight for k, v in cleaned_weights.items()}
            logger.info(
                f"[HRP_MODEL] Da chuan hoa lai trong so. Tong moi: {sum(cleaned_weights.values())}"
            )
        ...
        allocation_lp, leftover_lp = optimize_hrp_allocation(
            cleaned_weights, latest_prices, total_portfolio_value
        )
