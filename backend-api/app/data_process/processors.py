...
from app.data_process.fetchers import get_index_history
...
def get_market_indices_metrics(
    symbols: Iterable[str] = DEFAULT_INDEX_SYMBOLS, source: str = "VCI"
) -> List[dict]:
    ...
    for symbol in symbols:
        ...
        note = (
            f"KL {volume / 1_000_000:.1f} triệu cp"
            if pd.notna(volume)
            else "Chưa có số liệu thanh khoản"
        )
...
def summarize_sector_performance(
    snapshot: pd.DataFrame, top_n: Optional[int] = None
) -> pd.DataFrame:
...
def summarize_market_cap_distribution(
    snapshot: pd.DataFrame, top_n: int = 8
) -> pd.DataFrame:
...
def get_foreign_flow_leaderboard(
    snapshot: pd.DataFrame, top_n: int = 6
) -> pd.DataFrame:
    """Rank tickers by net foreign flow."""
    if (
        snapshot is None
        or snapshot.empty
        or "foreign_buysell_20s" not in snapshot.columns
    ):
        return pd.DataFrame(
            columns=[
                "ticker",
    ...
    df = snapshot[
        ["ticker", "industry", "foreign_buysell_20s", "avg_trading_value_20d"]
    ].copy()
    df["foreign_buysell_20s"] = pd.to_numeric(
        df["foreign_buysell_20s"], errors="coerce"
    ).fillna(0)
    df["avg_trading_value_20d"] = pd.to_numeric(
        df["avg_trading_value_20d"], errors="coerce"
    ).fillna(0)
    ...
    outflow = df.tail(max(top_n, 1)).sort_values(
        "foreign_buysell_20s"
    )  # Keep negative values at bottom
...
def get_liquidity_leaders(snapshot: pd.DataFrame, top_n: int = 30) -> pd.DataFrame:
    ...
    data["avg_trading_value_20d"] = pd.to_numeric(
        data.get("avg_trading_value_20d"), errors="coerce"
    )
    data["price_growth_1w"] = pd.to_numeric(
        data.get("price_growth_1w"), errors="coerce"
    )  # Daily
    data["market_cap"] = pd.to_numeric(data.get("market_cap"), errors="coerce")

    data = data.dropna(subset=["avg_trading_value_20d"])
    data = data.sort_values("avg_trading_value_20d", ascending=False).head(
        max(top_n, 1)
    )
    return data
...
def get_sector_heatmap_matrix(snapshot: pd.DataFrame, top_n: int = 6) -> pd.DataFrame:
    ...
    melted = performance.melt(
        ...
        value_vars=[
            "avg_growth_1w",
            "avg_liquidity",
        ],  # Dropped 1M (redundant with Daily)
        var_name="metric",
        value_name="value",
    )
    metric_labels = {
