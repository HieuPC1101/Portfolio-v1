...
def initialize_chatbot_session():
    ...
    if "chatbot" not in st.session_state:
        try:
            ...
            # Kiểm tra API key + model có hợp lệ không
            if (
                not OPENROUTER_API_KEY
                or OPENROUTER_API_KEY == "your-openrouter-api-key-here"
            ):
                st.session_state.chatbot = None
                st.session_state.chatbot_error = "Chưa cấu hình OPENROUTER_API_KEY"
            elif not OPENROUTER_MODEL:
                st.session_state.chatbot = None
                st.session_state.chatbot_error = "Chưa cấu hình OPENROUTER_MODEL"
            else:
...
def render_chatbot_sidebar(portfolio_context=None):
    ...
    if st.session_state.chatbot is None:
        with st.sidebar:
            ...
            st.info(
                "Vui lòng đặt biến môi trường `OPENROUTER_API_KEY` và `OPENROUTER_MODEL` hoặc khai báo trong `utils/secret_config.py`.\n\nLấy API key tại: https://openrouter.ai/keys"
            )
        return
    ...
    with chat_section:
        ...
        with chat_container:
            ...
            if (
                ...
                and st.session_state.show_quick_questions
                and not st.session_state.is_thinking
            ):
        ...
        user_input = st.chat_input(
            "Nhập câu hỏi của bạn...", disabled=st.session_state.is_thinking
        )

        if user_input and not st.session_state.is_thinking:
            handle_user_message(
                user_input, portfolio_context, chat_parent=chat_container
            )
            st.session_state.show_quick_questions = False
            st.rerun()
...
def get_current_portfolio_context():
    ...
    if (
        ...
        and st.session_state.optimization_result
    ):
...
def render_chatbot_page():
    ...
    if st.session_state.chatbot is None:
        ...
        st.info(
            "Vui lòng đặt biến môi trường OPENROUTER_API_KEY và OPENROUTER_MODEL hoặc khai báo trong utils/secret_config.py. Lấy API key tại: https://openrouter.ai/keys"
        )
        return
                ...
                for col_idx, question in enumerate(row_questions):
                    with cols[col_idx]:
                        if st.button(
                            ...
                            portfolio_context = get_current_portfolio_context()
                            handle_user_message(
                                question, portfolio_context, chat_parent=chat_container
                            )
                            st.session_state.show_quick_questions = False
                            st.rerun()
